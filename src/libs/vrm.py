...
class VRM_API:
    ...
    def __init__(self, username=None, password=None, demo=False):
        ...
        self.AGGR_STATS_ENDPOINT = self.API_ENDPOINT + '/v2/installations/{inst_id}/overallstats'
        ...
        self.OVERVIEW_ENDPOINT = self.API_ENDPOINT + '/v2/installations/{inst_id}/system-overview'
    ...
    def get_overview(self, inst_id):
        if not self._is_initialized():
            return None

        request_url = self.OVERVIEW_ENDPOINT.format(inst_id=inst_id)

        resp = self._send_query_request(request_url)
    ...
    def get_widget(self, inst_id, widget_name, instance):

        url = "https://vrmapi.victronenergy.com/v2/installations/{inst_id}/widgets/{widget_name}?instance={instance}"

        url = url.format(inst_id=inst_id, widget_name=widget_name, instance=instance)
        headers = {'X-Authorization': "Bearer %s" % self._auth_token}

        response = requests.get(url, headers=headers, params=None)    
        return response.json()

    def _state_graph_widgets(self, inst_id, widget_name, instance=None, start=None, end=None):
        """
        ...
        request_url = self.WIDGETS_ENDPOINT.format(inst_id=inst_id, widget_type=widget_name)
        widgets     = self._send_query_request(request_url, data_dict)
        return widgets
